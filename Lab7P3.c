...
char line1[16];     //character string for the printout
...
void main(void)
{
    ...
    while(1){
        ...
        commandWrite(0x06);
        for(i=0;i<16;i++)                       //Print "Labratory over to screen hold for 1 second
        {
            dataWrite(line1[i]);
        }
        systick_delay(1000);

        for(j=0;j<15;j++){                      //Reprint but skip letter n until all are gone
